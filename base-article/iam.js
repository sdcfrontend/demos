...
var iom = iom || (function () {
  var dummySite = "dummy",
      ...
      nt = 2,
      cookiewhitelist = ["infonlin", "housectr", "pro7", "sat1", "fem", "kabel1", "bild", "stylbook", "travelbo", "techbook", "fitbook"],
      socioToken = "ed2c3fc5e25875a4cb963e83726d5fd3",
      frequency = 60000,
      hbiAdShort = 5000,
      hbiAdMedium = 10000,
      hbiAdLong = 30000,
  ...
  function buildResult(params) {
    ...
    result.xy = getScreen();
    result.lo = "GB/London, City of";
    result.cb = "0016";
    result.i2 = "0016c9e8b952044e95a4cb963";
    result.ep = 1544511773;
    result.vr = "406";
    result.id = fingerprint();
    result.st = result.st ? result.st : dummySite;
    if (window.tizen !== undefined) {
      if (fetchTizenUUIDs() !== 'notizen') {
        result.mi = fetchTizenUUIDs();
      }
    }
    if (arrayContains(cookiewhitelist, result.st)) {
      result.i3 = FirstPartyCookie();
    }
    
    if (!result.cn && result.st.charAt(2) == "_") {
      cn = result.st.substr(0,2);
  ...
  function event(event) {
    ...
    if (inited && !isDoNotTrack() && (!checkEvents || (checkEvents && arrayContains(eventList, event)))) {
      ...
      if (arrayContains(['play', 'resm', 'alve', 'sfqt', 'ssqt', 'stqt'], event) && mode === 1 && result.hasOwnProperty('hb')) {
        startHeart();
      }
      return result;
    }
  ...
  function forwardToOewa(params) {
    createScriptTagAsync('//dispatcher.oewabox.at/oewa.js', function () {
      var OEWA = {
        "s": params.st,
        "cp": params.cp
      };
  ...
  function init(params,m) {
    ...
    inited = 1;
    forwardToOldSZM();
    if (result.cn && result.cn == "at") {
      forwardToOewa(params);
    }
    return {};
  }

...
function setProperties(subject, frameDimensions, property, defaults, index) {

  switch (property) {
    case 'fade':
    var max = 1*defaults.multipliers[index];
    ...
    defaults.actions.forEach(function(action) {
      ...
      switch (action) {
        case 'blur':
        var max = 10*defaults.multipliers[index];
        var value = framePercentageValue(frameDimensions, defaults, max);
        ...
        case 'saturate':
        var min = 0;
        var max = 100*defaults.multipliers[index];
        ...
        case 'hue-rotate':
        var max = 360*defaults.multipliers[index];
        var value = framePercentageValue(frameDimensions, defaults, max);
    ...
    defaults.actions.forEach(function(action) {
      ...
      switch (action) {
        case 'translateY':
        var value = defaults.direction*(frameDimensions.top*defaults.multipliers[index]);
  ...
  if (parallaxFrameDimensions.top < height) {
    ...
    parallaxSubjects.forEach(function(parallaxSubject) {
      ...
      if (!parallaxSubject.hasAttribute('passed')) {
        ...
        if (parallaxSubject.getAttribute('data-multipliers') != null) {
          defaults.multipliers = parallaxSubject.getAttribute('data-multipliers').split(',');
        }
        ...
        defaults.properties.forEach(function(property, index) {

          setProperties(parallaxSubject, parallaxFrameDimensions, property, defaults, index);
